/**
 * Jobber → Supabase: **jobs only** (no quotes or invoices API calls).
 * Uses CRM accounts + properties + quotes already in Supabase; only paginates Jobber `jobs`.
 *
 * Use when quote import already ran and jobs keep hitting rate limits — avoids re-fetching
 * hundreds of quotes before every jobs attempt.
 *
 *   node scripts/jobber-import-jobs-only.mjs
 *
 * After jobs succeed, run invoices (same token budget concern):
 *   node scripts/jobber-import-invoices-only.mjs
 *
 * Same .env.local as jobber-migrate.mjs. Or JOBBER_ACCESS_TOKEN=… to skip OAuth.
 */
